...
function read_data()
    ...
    times_distribution = Pickle.load("19_times_$num_scenarios.pkl")
...
function run_dynamic_programming(distance, profits, times_distribution, horizon_length)
    ...
    while new_labels_were_generated 
        ...
        for point in 1:num_points
            ...
            # print("Non-dominated labels for point ", point, " :")
            # for label in non_dom_labels[point]
            #     @printf(" (%d, %d, %.3f)", label.id, label.time, label.profit)
            # end
            # println()
